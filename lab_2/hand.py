...
class Neuron:
    ...
    def activate(self, inputs):
        self.inputs = inputs
        self.output = self.logistic_function(
...
class MultyLayerPerceptron:
    ...
    def predict(self, input_layer):
        ...
        outputs = [neuron.activate(layer_outputs)
    ...
    def study(self, inputs, correct_output):
        ...
        input_layer_delta = [
            neuron.get_delta for neuron in self.hidden_layers[0]]
...
def main():
    ...
    y_test = y_test.apply(lambda x: 0 if x <= 4 else (
        1 if (x >= 5 and x <= 6) else 2))
